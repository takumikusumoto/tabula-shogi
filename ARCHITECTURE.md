...
- **16-bit 量子化バイナリ形式 (`TABU_NN4`)**:
  - ヘッダー（8バイトマジック `TABU_NN4` ＋ 入出力次元）＋ 16bit整数重み（合計 645,908 bytes）
...
- **特徴量表現と次元数**:
  ...
  - 後手視点では盤面の点対称反転（`square ^ 80`）および駒の敵味方反転を行い、先後同一ネットワーク重みで完全に対称性を維持。
- **ネットワーク構造**:
  - **入力層**: 204,120 スパース入力（1局面あたりアクティブ特徴量は約40個）。
  - **隠れ層**: 128 ニューロン、活性化関数 ClippedReLU（$0 \le x \le 127$ または Float $0.0 \le x \le 1.0$）。
  - **出力層**: 256 入力（先手視点アキュムレータ128 ＋ 後手視点アキュムレータ128）➜ 1 スコア出力。
...
- **モデルバイナリ形式 (`TABU_HK1`)**:
  - ヘッダー（マジックバイト `TABU_HK1` ＋ 入出力次元）＋ 32bit Float重みバイナリ。
