...
- **自己対局強化学習パイプライン (Phase 3)**:
  - スタンドアロンCLIによる高速自己対局エンジンの構築。
  - 外部棋譜・重みを一切使わず、ゼロから生成した自己対局棋譜（CSA形式/独自バイナリ）の蓄積。
- **Texel Tuning (自己対戦によるHCE評価パラメータ自動最適化)**:
  - PSTや駒価値の係数を、自己対戦勝敗結果からロジスティック回帰で自動調整。
- **自己対局生成データによるNNUE学習**:
  - 自己対局で得られた棋譜データを用いたスクラッチNNUE重みの学習と自動更新ループ。
